...
// window.addEventListener('DOMContentLoaded', (event) => {
    ...
    class Network {
        constructor(inputs, layerSetupArray) {
            ...
            this.momentum = 0.000951565
            this.learningRate = 0.000951565
            ...
            for (let t = 0; t < this.nodeCount; t++) {
                let cluster = []
                let tv = Math.floor(Math.random() * (this.nodeCount + inputs.length) * 5) + 30
                for (let k = 0; k < tv; k++) {
                    let index = new Point(-1, -1)
                    if (Math.random() < .5 || t >= this.nodeCount - 3) {
                        ...
                        if (!cluster.includes(this.structure[index.x][index.y])) {
                            if (t >= this.nodeCount - 3) {
                                if (index.y < this.nodeCount - 3) {
    ...
    for (let t = 0; t < 44; t++) {
        ...
        zinputs.push(new Data(0))
    }
    let net = new Network(inputs, [8])
    let makk = 8
    let count = 0
    ...
    function main(inputObject) {
        ...
        if (keysPressed['n']) {
            net = new Network(zinputs, [makk*2])
            makk*=2
            keysPressed['n'] = false
            return
        }
        ...
        // game code goes here
        canvas_context.drawImage(pom, 0, 0, pom.width, pom.width, 0, 0, 51, 50)
        // return
        if (keysPressed['e']) {
            count += 1000
        }
